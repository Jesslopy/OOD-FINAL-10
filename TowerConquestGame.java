...
public class TowerConquestGame {
    ...
    private void removePreviousStrategyEffects() {
        if (playerStrategy instanceof AggressiveStrategy) {
            // Reverse the 1.5x multiplier on Strength and Wealth
            playerCharacter.setStrength((int)(playerCharacter.getStrength() / 1.5));
            playerCharacter.setWealth((int)(playerCharacter.getWealth() / 1.5));
        } else if (playerStrategy instanceof DefensiveStrategy) {
            // Reverse the 1.5x multiplier on Strength and Health
            playerCharacter.setStrength((int)(playerCharacter.getStrength() / 1.5));
            playerCharacter.setHealth((int)(playerCharacter.getHealth() / 1.5));
        } else if (playerStrategy instanceof DiplomaticStrategy) {
            // Reverse the 1.5x multiplier on Wealth and Influence
            playerCharacter.setWealth((int)(playerCharacter.getWealth() / 1.5));
            playerCharacter.setInfluence((int)(playerCharacter.getInfluence() / 1.5));
        }
        // Add similar logic for other strategies if any
    }
    ...
    private void changeStrategy() {
        Scanner scanner = new Scanner(System.in);
        System.out.println("Choose a new strategy (Aggressive, Defensive, Diplomatic): ");
        String newStrategyName = scanner.nextLine();
        removePreviousStrategyEffects();
        assignStrategy(newStrategyName);
        playerStrategy.executeStrategy(playerCharacter, gameState);
        gameState.setCurrentStrategy(playerStrategy); // Update GameState with the new strategy
        System.out.println("Strategy changed to " + newStrategyName);
    }
    ...
    public void startGame() {
        ...
        System.out.println("Your strategy can be changed during any week using the command: 'strategy',but be careful, you lose the chance to fortify your kingdom that week!");
        ...
        while (true) {
            ...
            System.out.println("Enter command for this week: strengthen, heal, mine, debate, strategy, exit");
